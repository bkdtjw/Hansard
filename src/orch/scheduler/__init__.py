...
M2 契约 §3 追加权限三件套 ensure_worktrees / audit_write_scope / autocommit）：
  ...
  ensure_worktrees(config, target_repo, worktrees_root, thread_id=None) -> dict[str, Path]
                                                         §8.1 worktree 隔离
  audit_write_scope(worktree, write_scope, last_ok_commit) -> tuple[bool, list[str]]
                                                         §8.2 diff 越权审计
  autocommit(worktree, role, event_id) -> str | None     §4.5 wip:{role}@E{n} 自动提交
