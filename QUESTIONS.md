...
Q9 [M5后/adapters] §5.6.3"大小写不敏感子串"口径对十六进制串的固有撞击风险，是否修订匹配口径
  ...
  裁决: 2026-07-26 用户裁决 → 采 A。维持 §5.6.3 子串口径，spec 与实现均零改动；
        残留的 stderr 十六进制撞击风险接受（误跳闸有 `orch adapter enable` 恢复路径），
        中文 pattern 兼容性为决定性论据。

Q10 [spec§0×§12] §0 行24"范围外（禁止实现）：图形界面"与 §12 行580"控制台**必须**提供
    enable/disable 开关按钮"正面冲突——宪法内部矛盾
  背景: 控制台 2026-07-05 以"spec 之外补充交付"落地（IMPLEMENTATION_NOTES W1 先例）；
        M5 增补把控制台写成规范要求（§12 行580 两处"必须"），但 SPEC-AMENDMENT-M5-draft
        的 A1–A10 十条修订均未触 §0，行24 的禁令原文未动。AionUi 对齐调研
        （aionui-alignment-study.md，wf_25c86736）发现并经 Lead 亲验两行原文属实，
        按 CLAUDE.md"发现 spec 内部矛盾：停下报告"立案。该矛盾卡住对齐四特性的
        全部施工（控制台扩展在宪法层名不正）。
  选项: A 修 §0 行24 一行：图形界面后加"（本地运维控制台除外，见 §12）"——
        消除矛盾，与 M5 已验收的既成事实一致，不牵动其他章节（推荐）；
        B 维持 §0 原文，控制台永久定位"spec 外补充交付"（W1 先例）——零改动，
        但 §12/§15 的"必须"永久悬空，控制台后续一切工作失去宪法依据。
  裁决: 2026-07-29 用户裁决 → 采 A。§0 行24「图形界面」后已补「（本地运维控制台
        除外，见 §12）」，一行修订消除矛盾；实现零改动。同日用户批准控制台对齐
        施工：先 T1(状态泡+通讯录单聊)+T2(待办+统计)+T3(config 缺陷卡)，
        T4(View Steps)待其两项前置问题另行裁决。

Q11 [console/§7.1×§14] invoke 内部执行过程（工具调用明细）可否在控制台作人类展示；
    连带 logs/ 原文合规缺口修复与暴露口径（T4 的两项前置）
  背景: AionUi 对齐调研 F1（aionui-alignment-study.md）：§7.1 行396「调度器不知道、
        也不需要知道信封背后是一步还是一百步——全系统最重要的抽象边界」主语是调度器，
        控制台把 invoke 内部工具调用**只作人类展示**（不回流任何调度判定）是否越界，
        spec 未定义、§17 不覆盖，须人裁。连带事实（修复无须裁决，属还债）：logs/ 今天
        落的是 str(raw_env)（core.py/async_core.py 的 write_invoke_log 调用）而非
        stdout 原文，与 §14 行603「每次 invoke 的完整输入/输出原文落 threads/t-xxx/
        logs/」直接冲突。附属取舍：原文含 sessionId 等敏感串（Q9 档案实证），经 HTTP
        给前端的暴露口径。
  选项: A 放行展示：仅人类展示、禁止回流任何调度判定；HTTP 端点只出**解析后的步骤
        摘要**（工具名+命令摘要截断+计数+耗时），stdout 原文不经 HTTP 直出（留盘上
        审计，orch attach/logs 现场勘查照旧）；
        B 不展示，控制台停留在"派发→回复"粒度；
        C 展示且原文直出（sessionId 等随之进页面）。
  裁决: 2026-07-30 用户直接指示「我要求的效果是和我之前上传的图片是差不多的……每一个
        聊天记录都需要看到具体的细节，某一个角色对话的细节都要看到」→ 展示放行，
        §7.1 边界按"主语是调度器、展示层不回流即不越界"落地。暴露粒度属 serve 实现
